...
const PurchaseOfficerLayout = ({
  ...
  const displayName = currentUser?.full_name || currentUser?.first_name || currentUser?.username || 'Purchase Officer';
  const employeeId = currentUser?.employee_id || 'PO-STAFF';
  const roleTitle = currentUser?.role || 'Purchase Officer';
  const usernameHandle = currentUser?.username ? `@${currentUser.username}` : '@purchase_officer';

  const navItems = [
    { name: 'Dashboard', path: '/purchase-officer/dashboard', icon: HiHome },
    { name: 'Purchase Requisitions', path: '/purchase-officer/requisitions', icon: HiClipboardList },
    { name: 'Procurement Opportunities', path: '/purchase-officer/opportunities', icon: HiTag },
    { name: 'Quotation Management', path: '/purchase-officer/quotations', icon: HiDocumentText },
    { name: 'Purchase Orders', path: '/purchase-officer/purchase-orders', icon: HiShoppingBag },
    { name: 'Vendor Management', path: '/purchase-officer/vendors', icon: HiOfficeBuilding },
    { name: 'Deliveries', path: '/purchase-officer/deliveries', icon: HiTruck },
    { name: 'Notifications', path: '/purchase-officer/notifications', icon: HiBell },
    { name: 'My Profile', path: '/purchase-officer/profile', icon: HiUserCircle },
  ];
    ...
    <div className="min-h-screen bg-[#F8F8FC] flex font-sans text-slate-800">
      ...
      >
        ...
        <div className="p-5 border-b border-violet-50 flex items-center justify-between">
          <Link to="/purchase-officer/dashboard" className="flex items-center gap-2.5">
        ...
        {/* Sidebar Footer - Officer Badge & Logout */}
        <div className="p-4 border-t border-violet-50 space-y-2.5">
          <div className="p-2.5 bg-violet-50/80 rounded-2xl border border-violet-100 flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-extrabold text-violet-900 leading-tight uppercase truncate">{roleTitle}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-violet-100 px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/purchase-officer/notifications"
              title="Notifications"
            ...
            >
              <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Purchase Officer Portal</p>
                </div>
              </div>

              <p className="text-sm text-slate-600 leading-relaxed">
                Are you sure you want to log out of the Purchase Officer Portal?
              </p>
...
export default PurchaseOfficerLayout;
