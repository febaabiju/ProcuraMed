...
const ProcurementCommitteeLayout = ({
  children,
  ...
  const currentUser = committeeUser || authUser;
  const displayName = currentUser?.full_name || currentUser?.first_name || currentUser?.username || 'Committee Member';
  const employeeId = currentUser?.employee_id || 'PC-STAFF';
  const roleTitle = currentUser?.role || 'Procurement Committee Member';
  const usernameHandle = currentUser?.username ? `@${currentUser.username}` : '@committee';

  const navItems = [
    { name: 'Dashboard', path: '/committee/dashboard', icon: HiHome },
    { name: 'Procurement Reviews', path: '/committee/reviews', icon: HiClipboardCheck },
    { name: 'Committee Decisions', path: '/committee/decisions', icon: HiCheckCircle },
    { name: 'Procurement History', path: '/committee/history', icon: HiArchive },
    { name: 'Notifications', path: '/committee/notifications', icon: HiBell },
    { name: 'My Profile', path: '/committee/profile', icon: HiUserCircle },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F8F8FC] flex font-sans text-slate-800">
      ...
      >
        ...
        {/* Sidebar Footer - Committee Badge & Logout */}
        <div className="p-4 border-t border-violet-50 space-y-2.5">
          <div className="p-2.5 bg-violet-50/80 rounded-2xl border border-violet-100 flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-extrabold text-violet-900 leading-tight uppercase truncate">Procurement Committee</p>
            ...
            >
              <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Procurement Committee Portal</p>
                </div>
              </div>

              <p className="text-sm text-slate-600 leading-relaxed">
                Are you sure you want to log out of the Procurement Committee Portal?
              </p>
...
export default ProcurementCommitteeLayout;
