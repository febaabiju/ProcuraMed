...
const VendorLayout = ({
  ...
  const usernameHandle = user?.username ? `@${user.username}` : '@vendor';

  const navItems = [
    { name: 'Dashboard', path: '/vendor/dashboard', icon: HiHome },
    { name: 'Supply Categories', path: '/vendor/supply-categories', icon: HiTag },
    { name: 'Purchase Requests / Opportunities', path: '/vendor/opportunities', icon: HiClipboardList },
    { name: 'Quotations', path: '/vendor/quotations', icon: HiDocumentText },
    { name: 'Purchase Orders', path: '/vendor/purchase-orders', icon: HiShoppingBag },
    { name: 'Deliveries', path: '/vendor/deliveries', icon: HiTruck },
    { name: 'Invoices', path: '/vendor/invoices', icon: HiCreditCard },
    { name: 'Messages / Notifications', path: '/vendor/notifications', icon: HiBell },
    { name: 'Company Profile', path: '/vendor/profile', icon: HiOfficeBuilding },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F8F8FC] flex font-sans text-slate-800">
      ...
      >
        ...
        {/* Sidebar Footer - Approved Vendor Badge & Logout */}
        <div className="p-4 border-t border-violet-50 space-y-2">
          <div className="p-2.5 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center gap-2">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-emerald-900 leading-tight uppercase">Approved Vendor</p>
              <p className="text-[9px] text-emerald-700 truncate">Portal Access Active</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="hidden sm:block text-left">
                ...
                <p className="text-[10px] font-semibold text-violet-600 mt-1">{usernameHandle}</p>
              </div>
            </Link>
            ...
            >
              <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">ProcuraMed Vendor Portal</p>
                </div>
              </div>
