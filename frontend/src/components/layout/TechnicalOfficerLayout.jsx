...
const TechnicalOfficerLayout = ({
  ...
  const displayName = currentUser?.full_name || currentUser?.first_name || currentUser?.username || 'Technical Officer';
  const employeeId = currentUser?.employee_id || 'TO-STAFF';
  const roleTitle = currentUser?.role || 'Technical Officer';
  const usernameHandle = currentUser?.username ? `@${currentUser.username}` : '@technical_officer';

  const navItems = [
    { name: 'Dashboard', path: '/technical-officer/dashboard', icon: HiHome },
    { name: 'Technical Evaluations', path: '/technical-officer/evaluations', icon: HiBeaker },
    { name: 'Assigned Reviews', path: '/technical-officer/assigned-reviews', icon: HiClipboardCheck },
    { name: 'Evaluation History', path: '/technical-officer/history', icon: HiArchive },
    { name: 'Notifications', path: '/technical-officer/notifications', icon: HiBell },
    { name: 'My Profile', path: '/technical-officer/profile', icon: HiUserCircle },
  ];
    ...
    <div className="min-h-screen bg-[#F8F8FC] flex font-sans text-slate-800">
      ...
      >
        ...
        <div className="p-5 border-b border-violet-50 flex items-center justify-between">
          <Link to="/technical-officer/dashboard" className="flex items-center gap-2.5">
        ...
        <div className="p-4 border-t border-violet-50 space-y-2.5">
          <div className="p-2.5 bg-violet-50/80 rounded-2xl border border-violet-100 flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-extrabold text-violet-900 leading-tight uppercase truncate">Technical Evaluation</p>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-violet-100 px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-xs">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-violet-50 text-violet-700 border border-violet-200 text-[10px] font-bold uppercase">
                  <HiShieldCheck className="w-3 h-3 text-violet-600" />
                  Technical Officer
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/technical-officer/notifications"
              title="Notifications"
            ...
            >
              <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Technical Officer Portal</p>
                </div>
              </div>

              <p className="text-sm text-slate-600 leading-relaxed">
                Are you sure you want to log out of the Technical Officer Portal?
              </p>
...
export default TechnicalOfficerLayout;
